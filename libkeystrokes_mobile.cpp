...
//  Offsets
// ══════════════════════════════════════════════════════════════════════════
static const uintptr_t g_normalTickOff = 0x8a550bc; // confirmed: prologue 0x1e8 before mMove read
...
struct MoveInputComponent {
    ...
    uint8_t  mFlagValues;  // [0x60] low byte of brstd::bitset<11,ushort>; mIsPaddling at [0x61]
    bool     mIsPaddling[2];    // [0x61]
};
static_assert(offsetof(MoveInputComponent, mMove_x)     == 0x24, "mMove_x");
static_assert(offsetof(MoveInputComponent, mFlagValues) == 0x60, "mFlagValues");
static_assert(offsetof(MoveInputComponent, mIsPaddling) == 0x61, "mIsPaddling");
...
static MoveInputComponent* getMoveInput_entt(void* lp) {
    ...
    // registry internal storage must be readable — guards against freed registry
...
static bool mic_valid(uintptr_t ptr) {
    ...
    if (m->mFlagValues > 0xFF) return false;  // uint8 low byte of 11-bit bitset
    return true;
}
...
static void updateKeysFromPlayer() {
    ...
    if (!mic) {
        // Count consecutive failures — clear stale player pointer after limit
    ...
    g_lpFailCount = 0; // reset on success

    float mx   = mic->mMove_x;
    float my   = mic->mMove_y;
    bool  jump = (mic->mInputState[0] & 0x1) != 0  // jump: bit 0 of MoveInputState
             || (mic->mFlagValues       & 0x1) != 0; // bit 0 of flagset
...
static void findMcBase() {
    ...
    while (fgets(line, sizeof(line), f)) {
        if (strstr(line, "libminecraftpe.so")) {
            ...
            if (!g_mcBase) { g_mcBase=s; g_baseFound=true;
